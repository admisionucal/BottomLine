...
// ============================================================
// LOGIN — por columna "Usuario"; Email sigue siendo el ID interno
// ============================================================
...
function getLeads(body) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (pagoInfo) {
      leadObj['FECHA DE PAGO COMPLETO'] = pagoInfo['FECHA DE PAGO COMPLETO'] || '';
      leadObj['FECHA DE PROMESA DE PAGO'] = pagoInfo['FECHA DE PROMESA DE PAGO'] || '';
    }
    ...
    for (var bk in bottomRow) if (bottomRow.hasOwnProperty(bk)) leadObj[bk] = bottomRow[bk];
...
function getLeadDetail(body) {
  ...
  if (hoyOverrideDetail) {
    ...
    if (esValorMerge(hoyOverrideDetail['NUMERO DE DOCUMENTO'])) leadObj['NUMERO DE DOCUMENTO'] = hoyOverrideDetail['DNI'];
  }
...
function getEmailLeadCC(campana, idPrometeo) {
  ...
  if (hoyOverride && esValorMerge(hoyOverride['EMAIL'])) return hoyOverride['EMAIL'];
...
function enviarCC(body) {
  ...
  if (!resultado.success) {
    conLock(function() {
      revertirClaimCC(claim.rowIndex, claim.headers);
      return { success: true };
    });
  }
...
function enviarCCClaim(body) {
  ...
  // Revalida STATUS === PENDIENTE justo antes de enviar: evita doble envío
  // por doble clic o dos pestañas abiertas del mismo supervisor.
  if (String(solicitud.STATUS).trim() !== 'PENDIENTE') {
...
function revertirClaimCC(rowIndex, headers) {
  var sheet = getHojaSolicitudesCC();
  var statusIdx = headers.indexOf('STATUS');
  sheet.getRange(rowIndex + 1, statusIdx + 1).setValue('PENDIENTE');
}
...
function saveBottomInterno(body) {
  ...
  if (tocaPerfil && rowIndex !== -1) {
      var histIdx = headers.indexOf('COMENTARIOS_HISTORIAL');
      if (histIdx !== -1) {
          var historialActual = parsearHistorial(data[rowIndex - 1][histIdx]);
          var snapshotAnterior = {};
          CAMPOS_PERFIL.forEach(function(c) {
              snapshotAnterior[c] = data[rowIndex - 1][headers.indexOf(c)] || '';
          });
...
function capturarIdentidadBase(campana, idPrometeo) {
  var sheetBase = SS.getSheetByName(campana);
  if (!sheetBase) return { nombre: '', dni: '', celular: '', carrera: '' };

  var headersBase = sheetBase.getRange(1, 1, 1, sheetBase.getLastColumn()).getValues()[0].map(function(h){ return String(h).trim(); });
  var idIdx = headersBase.indexOf('ID PROMETEO');
  if (idIdx === -1) return { nombre: '', dni: '', celular: '', carrera: '' };

  var lastRow = sheetBase.getLastRow();
  if (lastRow < 2) return { nombre: '', dni: '', celular: '', carrera: '' };

  var colLetter = columnToLetter(idIdx + 1);
  var rangoColumna = sheetBase.getRange(colLetter + '2:' + colLetter);
  var encontrado = rangoColumna.createTextFinder(String(idPrometeo).trim())
                      .matchEntireCell(true)
                      .findNext();
  if (!encontrado) return { nombre: '', dni: '', celular: '', carrera: '' };

  var rowNum = encontrado.getRow();
  var rowValues = sheetBase.getRange(rowNum, 1, 1, headersBase.length).getValues()[0];
