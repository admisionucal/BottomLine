...
function statusEsVivo(status) {
  var s = String(status || '').trim();
  return s === 'VALORES_VALORACIONES_POSITIVAS_VIVA' || s === 'VALORES_PROMESA_DE_PAGO_VIVA';
}

function hoyOverrideEsConfiable(hoyOverride, asesorBaseRaw, encontradoEnBase, statusBaseRaw) {
  if (!hoyOverride) return false;
  if (!statusEsVivo(hoyOverride['STATUS DE GESTION'])) return false;

  if (!encontradoEnBase) return true;

  if (!statusEsVivo(statusBaseRaw)) return true;
...
function permitirActualizarAsignacionDetail(hoyOverride, asesorBaseRaw, encontradoEnBase, statusBaseRaw) {
  if (!hoyOverride) return false;

  if (!statusEsVivo(hoyOverride['STATUS DE GESTION'])) return true;
  if (!encontradoEnBase) return true;
  if (!statusEsVivo(statusBaseRaw)) return true;
...
function getLeads(body) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (hoyOverride) {
      var statusBaseValue = statusValue;
      if (esValorMerge(hoyOverride['STATUS DE GESTION'])) statusValue = hoyOverride['STATUS DE GESTION'];
      if (permitirActualizarAsignacionDetail(hoyOverride, asesorValue, true, statusBaseValue) && esValorMerge(hoyOverride['ASESOR ULT TIP DF SN CONTC'])) {
        asesorValue = hoyOverride['ASESOR ULT TIP DF SN CONTC'];
      }
...
function getLeadDetail(body) {
  ...
  var statusBaseDetail = leadObj['STATUS DE GESTION'];
  ...
  if (!encontradoEnBaseDetail && hoyOverrideDetail) {
    ...
    if (leadObj['NOMBRE DEL COLEGIO'] && !leadObj['COLEGIO']) leadObj['COLEGIO'] = leadObj['NOMBRE DEL COLEGIO'];
  } else if (encontradoEnBaseDetail && hoyOverrideDetail) {
    if (esValorMerge(hoyOverrideDetail['STATUS DE GESTION'])) leadObj['STATUS DE GESTION'] = hoyOverrideDetail['STATUS DE GESTION'];
    if (esValorMerge(hoyOverrideDetail['NOMBRE DEL COLEGIO'])) {
      leadObj['NOMBRE DEL COLEGIO'] = hoyOverrideDetail['NOMBRE DEL COLEGIO'];
      leadObj['COLEGIO'] = hoyOverrideDetail['NOMBRE DEL COLEGIO'];
    }
    if (esValorMerge(hoyOverrideDetail['CODIGO MODULAR'])) leadObj['CODIGO MODULAR'] = hoyOverrideDetail['CODIGO MODULAR'];
    if (esValorMerge(hoyOverrideDetail['BOLETA DE COLEGIO'])) leadObj['BOLETA DE COLEGIO'] = hoyOverrideDetail['BOLETA DE COLEGIO'];
    if (esValorMerge(hoyOverrideDetail['FECHA COMPROMISO DE PAGO'])) leadObj['FECHA COMPROMISO DE PAGO'] = hoyOverrideDetail['FECHA COMPROMISO DE PAGO'];
  }

  // Identidad (nombre/teléfono/email/DNI/carrera)
  if (hoyOverrideEsConfiable(hoyOverrideDetail, leadObj['ASESOR_NOMBRE_RAW'], encontradoEnBaseDetail, statusBaseDetail)) {
  ...
  // Asignación (ASESOR / MODALIDAD / MODALIDAD INGRESO)
  var permitirAsignacion = permitirActualizarAsignacionDetail(hoyOverrideDetail, leadObj['ASESOR_NOMBRE_RAW'], encontradoEnBaseDetail, statusBaseDetail);
...
function getSolicitudesCC(body) {
  ...
  var resolvedBottomPorCampana = {}; // memoiza getBottomMapActivo por campaña dentro de este request
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    obj['MODALIDAD_LEAD'] = basicos.modalidad || '';
    obj['MODALIDAD_INGRESO_LEAD'] = basicos.modalidadIngreso || '';
    obj['CELULAR_LEAD'] = basicos.celular || '';
...
function combinarImagenesEnPdfCC(blobsImagenes, nombreArchivo) {
  ...
  try {
    ...
    var altoSlot = (altoUtil - gap * (n - 1)) / n;
    ...
    for (var i = 0; i < n; i++) {
      ...
      table.setColumnWidth(0, anchoUtil);
...
// Garantiza que un archivo (imagen o PDF) termine como blob PDF
...
function solicitarCCInterno(body) {
  ...
  // No se puede solicitar el envío de Condiciones Comerciales si el lead no tiene los datos de boleta guardada
  ...
  function casoIngresoPorModalidad(modalidadIngreso) {
    var t = String(modalidadIngreso || '').trim().toLowerCase();
    if (t.indexOf('sin conva') !== -1) return 3;
    if (t.indexOf('con conva') !== -1) return 2;
    return 1;
  }
  function vacioCC(v) { return v === undefined || v === null || String(v).trim() === ''; }

  var caso = casoIngresoPorModalidad(body.modalidadIngreso);
  var campos = filaBottomAsesor || {};
  var camposFaltantes = [];

  if (vacioCC(campos.TIPO_ALUMNO)) camposFaltantes.push('Tipo de Alumno');
  if (vacioCC(campos.NUMERO_CUOTAS)) camposFaltantes.push('Número de Cuotas');
  if (vacioCC(campos.DESCUENTO_PRECIOS)) camposFaltantes.push('Descuento Admisión y Matrícula');
  if (vacioCC(campos.BOLETA)) camposFaltantes.push('Boleta / Beneficio');
  if (vacioCC(campos.METODO_PAGO)) camposFaltantes.push('Método de Pago');  

  if (caso === 2 || caso === 3) {
    if (vacioCC(campos.TIPO_INSTITUCION_PROCEDENCIA)) camposFaltantes.push('Tipo de Institución de Procedencia');
    if (vacioCC(campos.INSTITUCION_PROCEDENCIA)) camposFaltantes.push('Institución de Procedencia');
    if (vacioCC(campos.CARRERA_PROCEDENCIA)) camposFaltantes.push('Carrera de Procedencia');
    if (vacioCC(campos.BOLETA_PROCEDENCIA)) camposFaltantes.push('Boleta de Procedencia');
    if (caso === 2 && vacioCC(campos.TIEMPO_OFRECIDO)) camposFaltantes.push('Tiempo Ofrecido');
    if (caso === 2 && vacioCC(campos.CICLO_QUEDO)) camposFaltantes.push('¿En qué ciclo se quedó?');
  }

  if (camposFaltantes.length > 0) {
    return json({ success: false, error: 'No se puede solicitar el envío de Condiciones Comerciales. Falta guardar en la ficha: ' + camposFaltantes.join(', ') + '.' });
  }
...
function getEmailLeadCC(campana, idPrometeo) {
  ...
  var statusIdx = headersBase.indexOf('STATUS DE GESTION');
  ...
  if (encontrado) {
    ...
    statusBase = statusIdx !== -1 ? sheetBase.getRange(encontrado.getRow(), statusIdx + 1).getValue() : '';
  }

  var hoyOverride = getHoyMap(campana)[String(idPrometeo).trim()];
  if (hoyOverrideEsConfiable(hoyOverride, asesorBase, !!encontrado, statusBase) && esValorMerge(hoyOverride['EMAIL'])) return hoyOverride['EMAIL'];
...
function enviarCCClaim(body) {
  ...
  // Evita doble envío por doble clic o dos pestañas abiertas del mismo supervisor.
  // - PENDIENTE
  // - ENVIADO
  // - PROCESANDO
...
function enviarCCProcesar(body, claim) {
  ...
  // Lineamientos (5 cuotas): llega en base64 desde el frontend (assets/), opcional — si falla o no llega, se envía solo con el PDF de CC.
  ...
  // Destinatarios: por defecto, email del lead + CORREOS_ADICIONALES de la solicitud.
...
function capturarIdentidadBase(campana, idPrometeo) {
  var vacio = { nombre: '', dni: '', celular: '', carrera: '', modalidad: '', modalidadIngreso: '' };
  var idTarget = String(idPrometeo).trim();

  var nombre = '', dni = '', celular = '', carrera = '', modalidad = '', modalidadIngreso = '', asesorBase = '', statusBase = '';
  ...
  if (sheetBase) {
    ...
    if (idIdx !== -1 && lastRow >= 2) {
      ...
      if (encontrado) {
        ...
        var modalidadIdx = headersBase.indexOf('MODALIDAD');
        var modalidadIngresoIdx = headersBase.indexOf('MODALIDAD INGRESO');
        var asesorIdx = headersBase.indexOf('ASESOR ULT TIP DF SN CONTC');
        var statusIdx = headersBase.indexOf('STATUS DE GESTION');
        ...
        modalidad = modalidadIdx !== -1 ? rowValues[modalidadIdx] : '';
        modalidadIngreso = modalidadIngresoIdx !== -1 ? rowValues[modalidadIngresoIdx] : '';
        asesorBase = asesorIdx !== -1 ? rowValues[asesorIdx] : '';
        statusBase = statusIdx !== -1 ? rowValues[statusIdx] : '';
      }
    }
  }

  // Override de hoy{campana} o si es VP/PP VIVA y es del mismo asesor se confía en hoy sin exigir asesor igual.
  var hoyOverride = getHoyMap(campana)[idTarget];
  if (hoyOverrideEsConfiable(hoyOverride, asesorBase, encontradoEnBase, statusBase)) {
    ...
    if (esValorMerge(hoyOverride['MODALIDAD'])) modalidad = hoyOverride['MODALIDAD'];
    if (esValorMerge(hoyOverride['MODALIDAD INGRESO'])) modalidadIngreso = hoyOverride['MODALIDAD INGRESO'];
  }

  if (!encontradoEnBase && !hoyOverride) return vacio;

  return { nombre: nombre, dni: dni, celular: celular, carrera: carrera, modalidad: modalidad, modalidadIngreso: modalidadIngreso };
}
...
// Filtrar con "Nombre"; mostrar "Nombre_Aux"
...
// Convierte un Nombre completo a su Nombre_Aux
...
function calcularResumenVpPpCampana(campana, esAdmin, nombreAsesor) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (hoyOverride) {
      var statusBaseValue = statusValue;
      if (esValorMerge(hoyOverride['STATUS DE GESTION'])) statusValue = hoyOverride['STATUS DE GESTION'];
      if (permitirActualizarAsignacionDetail(hoyOverride, asesorValue, true, statusBaseValue) &&
          esValorMerge(hoyOverride['ASESOR ULT TIP DF SN CONTC'])) {
        asesorValue = hoyOverride['ASESOR ULT TIP DF SN CONTC'];
      }
    }
