...
const CAMPOS_ASESOR = [
  ...
  'QUE_LE_FALTA', 'OTRAS_OPCIONES', 'DOLOR_NECESIDAD',
];
const CAMPO_SUPERVISOR = 'ACCIONES_DEFINIDAS';

export function calcularPerfilamientoCompleto(bottomRowUpperKeys: Record<string, any>, esAdmin = false) {
  ...
  const supervisorCompleto = !!(bottomRowUpperKeys[CAMPO_SUPERVISOR] && String(bottomRowUpperKeys[CAMPO_SUPERVISOR]).trim() !== '');

  const estado = esAdmin
    ? (supervisorCompleto ? 'Completo' : asesorCompleto ? 'Pendiente Supervisor' : 'Pendiente Asesor')
